  ...
  do
    ...
    for (( i = 1; i <=$ATLASSIZE; i++)) 
        ...
        #Candidates generation
      	# Registration
      	antsRegistrationSyNQuickDownSampledFactor2.sh -t "$TRANSFORMTYPE" -n 8 -d 3 -f $INPUTPATH/img"$target".nii -m $INPUTPATH/img"$i".nii -o $OUTPUTPATH/"reg"$i"t"$target
        ...
        LABLE_STR="${LABLE_STR} ${OUTPUTPATH}/cand${i}t${target}.nii"        
    done
    case $LABELFUSION in
      "MajorityVoting")
       ImageMath 3 "${OUTPUTPATH}/voting${target}".nii MajorityVoting $LABLE_STR
        ;;
      "LabelFusion")
