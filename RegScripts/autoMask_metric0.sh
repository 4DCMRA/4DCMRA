...
RIGIDCONVERGENCE="[1000x500x250x100,1e-7,20]"
RIGIDSHRINKFACTORS="8x4x2x1"
RIGIDSMOOTHINGSIGMAS="4x2x1x0vox"

AFFINECONVERGENCE="[1000x500x250x100,1e-7,15]"
AFFINESHRINKFACTORS="8x4x2x1"
AFFINESMOOTHINGSIGMAS="4x2x1x0vox"

SYNCONVERGENCE="[100x70x50x20,1e-7,10]"
SYNSHRINKFACTORS="8x4x2x1"
SYNSMOOTHINGSIGMAS="4x2x1x0vox"

if [[ $ISLARGEIMAGE -eq 1 ]];
  then
    RIGIDCONVERGENCE="[1000x500x100,1e-6,10]"
    RIGIDSHRINKFACTORS="8x4x2"
    RIGIDSMOOTHINGSIGMAS="4x3x2vox"

    AFFINECONVERGENCE="[1000x500x100,1e-6,10]"
    AFFINESHRINKFACTORS="8x4x2"
    AFFINESMOOTHINGSIGMAS="4x3x2vox"

    SYNCONVERGENCE="[300x200x100x50,1e-6,10]"
    SYNSHRINKFACTORS="8x4x2x1"
    SYNSMOOTHINGSIGMAS="4x2x1x1vox"
  fi
tx=Rigid
...
RIGIDSTAGE="--initial-moving-transform [${FIXEDIMAGES[0]},${MOVINGIMAGES[0]},1] \
            --transform ${tx}[0.1] \
            --metric MI[${FIXEDIMAGES[0]},${MOVINGIMAGES[0]},1,32,Random,0.25] \
            --convergence $RIGIDCONVERGENCE \
            --shrink-factors $RIGIDSHRINKFACTORS \
            --smoothing-sigmas $RIGIDSMOOTHINGSIGMAS"

AFFINESTAGE="--transform Affine[0.1] \
             --metric MI[${FIXEDIMAGES[0]},${MOVINGIMAGES[0]},1,32,Random,0.5] \
...
for(( i=0; i<${#FIXEDIMAGES[@]}; i++ ))
  do
    SYNMETRICS="$SYNMETRICS --metric
      MI[${FIXEDIMAGES[$i]},${MOVINGIMAGES[$i]},1,${CCRADIUS},Random,0.2]"
  done
