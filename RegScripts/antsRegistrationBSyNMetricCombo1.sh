...
RIGIDSTAGE="--initial-moving-transform [${FIXEDIMAGES[0]},${MOVINGIMAGES[0]},1] \
            --transform ${tx}[0.1] \
            --metric MeanSquares[${FIXEDIMAGES[2]},${MOVINGIMAGES[2]},1,1] \
            --convergence $RIGIDCONVERGENCE \
            --shrink-factors $RIGIDSHRINKFACTORS \
            --smoothing-sigmas $RIGIDSMOOTHINGSIGMAS"

AFFINESTAGE="--transform Affine[0.1] \
             --metric CC[${FIXEDIMAGES[1]},${MOVINGIMAGES[1]},1,5,Random,0.5] \
...
#   do
    ...
    SYNMETRICS="$SYNMETRICS --metric
      Demons[${FIXEDIMAGES[1]},${MOVINGIMAGES[1]},0.5]"
    SYNMETRICS="$SYNMETRICS --metric
      Demons[${FIXEDIMAGES[2]},${MOVINGIMAGES[2]},0.5]"
  # done
